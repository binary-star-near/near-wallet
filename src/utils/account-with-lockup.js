...
async function signAndSendTransaction(signAndSendTransactionOptions) {
    const { available: balance } =
        await this.wrappedAccount.getAccountBalance();
    ...
    const total = actions
        .map(
            (action) =>
                action?.transfer?.deposit || action?.functionCall?.deposit
        )
    ...
    const missingAmount = total
        .sub(new BN(balance))
        .add(new BN(MIN_BALANCE_FOR_GAS));
...
async function deleteLockupAccount(lockupAccountId) {
    console.info(
        "Destroying lockup account to claim remaining funds",
        lockupAccountId
    );
...
export async function transferAllFromLockup(missingAmount) {
    ...
    if (liquidBalance.gt(new BN(0))) {
        console.info(
            "Attempting to transfer from lockup account ID:",
            lockupAccountId
        );
        await this.wrappedAccount.functionCall({
            contractId: lockupAccountId,
...
async function getAccountBalance(limitedAccountData = false) {
    ...
    let stakingDeposits = await fetch(
        ACCOUNT_HELPER_URL + "/staking-deposits/" + this.accountId
    ).then((r) => r.json());
    ...
    await Promise.all(
        stakingDeposits.map(async ({ validator_id }) => {
            ...
            stakedBalanceMainAccount =
                stakedBalanceMainAccount.add(validatorBalance);
        })
    );
    ...
    try {
        ...
        const { transfer_poll_account_id, transfers_timestamp } =
            transferInformation;
        ...
        transfersTimestamp =
            transfersTimestamp || (Date.now() * 1000000).toString();
        const { code_hash: lockupContractCodeHash } =
            await lockupAccount.state();
            ...
            if (vestingInformation) {
                if (vestingInformation.unvestedAmount) {
                    ...
                    if (dateNowBN.lt(vestingInformation.vestingCliff)) {
                        ...
                        let timeLeft =
                            vestingInformation.vestingEnd.sub(dateNowBN);
        ...
        const stakingPoolLockupAccountId =
            await this.wrappedAccount.viewFunction(
                lockupAccountId,
                "get_staking_pool_account_id"
            );
        ...
        const isAccDeletable =
            lockedAmount.isZero() && stakedBalanceLockup.isZero();
        const MIN_BALANCE_FOR_STORAGE = getLockupMinBalanceForStorage(
        ...
        const liquidOwnersBalanceTransfersEnabled = isAccDeletable
            ...
            : BN.min(
                  ownersBalance,
                  new BN(lockupBalance.total).sub(
                      new BN(MIN_BALANCE_FOR_STORAGE)
                  )
              );
