...
const { fetchTokenFiatValues, fetchTokenUSDNFiatValues } =
...
class Routing extends Component {
    ...
    componentDidUpdate(prevProps) {
        const { activeLanguage, account, getAccountHelperWalletState } =
            this.props;

        if (
            prevProps.account.localStorage?.accountId !==
            account.localStorage?.accountId
        ) {
            this.setState({
                isInactiveAccount: getAccountIsInactive(
        ...
        if (
            prevProps.account.accountHelperWalletState.isLoaded !==
            account.accountHelperWalletState.isLoaded
        ) {
            const needsDeposit =
                account.accountHelperWalletState.fundedAccountNeedsDeposit;
            const accountId =
                account.accountId || account.localStorage?.accountId;
                    ...
                    <ThemeProvider theme={theme}>
                        ...
                        <Switch>
                            ...
                            <GuestLandingRoute
                                ...
                                render={(props) =>
                                    isInactiveAccount ? (
                                        <ActivateAccountWithRouter {...props} />
                                    ) : (
                                        <Wallet
                                            tab={tab}
                                            setTab={setTab}
