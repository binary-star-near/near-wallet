    ...
    STAKING: {
            ...
            await Promise.all(
                validators.map(async (validator, i) => {
                    try {
                        ...
                        if (
                            e.message.indexOf("cannot find contract code") ===
                            -1
                        ) {
                            console.warn(
        ...
        UPDATE_LOCKUP: async (
            ...
            const deposited = new BN(
                await contract.get_known_deposited_balance()
            );
            ...
            try {
                ...
                totalStaked = totalStaked.add(
                    new BN(validator.staked || ZERO.clone())
                );
                totalUnclaimed = totalUnclaimed.add(
        ...
        GET_LOCKUP: [
            async ({ accountId }) => {
                ...
                try {
                    await (
                        await new Account(wallet.connection, lockupId)
                    ).state();
            ...
            if (!accountIds) {
                ...
                const networkId =
                    ...
                    -1
                        ? MAINNET
                        : TESTNET;
                const allStakingPools = await fetch(
                    `${ACCOUNT_HELPER_URL}/stakingPools`
                ).then((r) => r.json());
                const prefix = getValidatorRegExp(networkId);
                accountIds = [
                    ...new Set([...rpcValidators, ...allStakingPools]),
                ].filter(
...
export const handleStakingUpdateAccount =
    ...
    async (dispatch, getState) => {
        const { accountId, balance } = exAccountId
            ? selectAllAccountsByAccountId(getState(), {
...
export const handleStakingUpdateLockup =
    (exAccountId) => async (dispatch, getState) => {
        ...
        const validators = selectStakingAllValidators(getState()).map(
            (validator) => ({
                ...validator,
            })
        );
...
export const handleStakingAction =
    (action, validatorId, amount) => async (dispatch, getState) => {
        ...
        const currentAccountId = selectStakingCurrentAccountAccountId(
            getState()
        );
        ...
        if (isLockup) {
            ...
            const contract = selectStakingFindContractByValidatorId(
                getState(),
                { validatorId }
            );
            await dispatch(
                staking[action].account(
                    validatorId,
                    amount,
                    accountId,
                    contract
...
export const updateStaking =
    (currentAccountId, recentlyStakedValidators) =>
    async (dispatch, getState) => {
        ...
        let currentAccount = selectStakingCurrentAccountbyAccountId(
            getState(),
            { accountId: currentAccountId }
        );

        if (!currentAccount) {
            currentAccount = selectStakingCurrentAccountbyAccountId(
                getState(),
                { accountId }
            );
...
export const handleUpdateCurrent =
